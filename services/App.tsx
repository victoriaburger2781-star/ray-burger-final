...
export default function App() {
        ...
        setProfile(prev => {
            ...
            if (newPoints >= 10) {
                ...
                return {
                    ...prev,
                    points: 10,
                    rewardsAvailable: prev.rewardsAvailable + 1
                };
            }
  ...
  const renderContent = () => {
    switch (currentView) {
          ...
          <div className="flex flex-col items-center justify-center flex-1 text-center space-y-8 animate-[fadeIn_0.5s] w-full px-4 pb-24">
            <div className="relative w-full max-w-2xl mt-4">
                ...
                <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-white/5">
                    <img 
                        ...
                        alt="Ray Burger Background" 
                        className="w-full h-72 object-cover opacity-80 hover:scale-105 transition-transform duration-700"
                    />
            ...
            <div className="grid grid-cols-1 gap-4 w-full max-w-xs relative z-10">
              <button 
                ...
                className="bg-white text-black hover:bg-zinc-200 font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-3 transition-all hover:scale-105 shadow-[0_0_20px_rgba(255,255,255,0.1)] group"
              >
                <div className="bg-orange-100 p-2 rounded-full group-hover:bg-orange-200 transition-colors">
                    <User className="text-orange-600" size={24} />
                </div>
                <div className="text-left">
                    <span className="block text-xs text-zinc-500 uppercase font-bold">Acceso</span>
                    <span className="block text-lg leading-none">{profile ? 'Mi Membresía' : 'Soy Cliente'}</span>
                </div>
              </button>

              <button 
                 onClick={() => setCurrentView(AppView.MENU)}
                 className="bg-zinc-800 text-white hover:bg-zinc-700 font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-3 border border-zinc-700 transition-transform hover:scale-105"
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                  <button 
                     onClick={() => setCurrentView(AppView.AI_CHEF)}
                     className="bg-gradient-to-br from-orange-900/80 to-red-900/80 backdrop-blur-sm text-white font-bold py-4 px-4 rounded-xl flex flex-col items-center justify-center gap-2 border border-red-800/50 transition-transform hover:scale-105"
                  >
                    <ChefHat className="text-orange-300" size={24} />
                    <span className="text-xs">Chef AI</span>
                  </button>
                  <button 
                     onClick={() => setCurrentView(AppView.SUGGESTIONS)}
                     className="bg-zinc-800/80 text-white font-bold py-4 px-4 rounded-xl flex flex-col items-center justify-center gap-2 border border-zinc-700 transition-transform hover:scale-105 hover:bg-zinc-700"
                  >
              ...
              <div className="mt-4 pt-4 border-t border-zinc-800 w-full flex flex-col items-center gap-2 text-zinc-500">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                      <MapPin size={12} />
                      <span>Av. Principal, Zona Grill</span>
                  </div>
              </div>

              <div className="mt-2 w-full flex justify-center">
                 <button 
                    onClick={() => setCurrentView(AppView.ADMIN_SCANNER)}
                    className="text-zinc-600 text-xs flex items-center gap-1 hover:text-zinc-400"
                >
                    <Scan size={12} /> Acceso Staff
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="w-full max-w-md px-4 pb-24 animate-[slideUp_0.4s]">
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        <div>
                            <label className="block text-xs font-bold text-zinc-500 uppercase mb-1 ml-1">Nombre</label>
                            <input 
                                type="text"
                                value={regName}
                                onChange={(e) => setRegName(e.target.value)}
                                className="w-full bg-black border border-zinc-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-orange-500 transition-colors"
                                placeholder="Tu nombre"
                                required
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-zinc-500 uppercase mb-1 ml-1">Apellido</label>
                            <input 
                                type="text"
                                value={regLastName}
                        ...
                        <button 
                            type="submit"
                            className="w-full bg-orange-600 hover:bg-orange-500 text-white font-bold py-4 rounded-xl mt-4 flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(234,88,12,0.3)]"
                        >
        ...
        return (
          <div className="flex flex-col items-center gap-6 w-full px-4 pb-28 animate-[slideUp_0.4s]">
            ...
            <div className="w-full text-center pt-2 pb-2">
                <h2 className="text-xl text-zinc-400 brand-font tracking-widest mb-0">BIENVENIDO</h2>
                <h1 className="text-5xl text-orange-500 brand-font drop-shadow-[0_0_15px_rgba(234,88,12,0.4)] leading-none uppercase truncate px-4">
                    {profile.name}
                </h1>
            </div>
            ...
            <div className="w-full max-w-md space-y-3">
                <h3 className="text-white font-bold text-sm ml-1 flex items-center gap-2">
                ...
                {profile.points >= 10 ? (
                    <div className="bg-gradient-to-r from-yellow-600 to-orange-600 p-0.5 rounded-xl shadow-[0_0_15px_rgba(234,179,8,0.3)] animate-pulse">
                        <div className="bg-black rounded-[10px] p-4 flex justify-between items-center relative overflow-hidden">
                            ...
                            <div className="pl-4">
                                <span className="block text-xs text-yellow-500 font-bold uppercase tracking-wider">¡Desbloqueado!</span>
                                <span className="text-2xl brand-font text-white">BURGER 2X1 GRATIS</span>
                                <p className="text-[10px] text-zinc-400">Válido en sucursal. Muestra este cupón.</p>
                            </div>
                            <button 
                                onClick={resetPoints}
                                className="bg-white text-black text-xs font-bold px-4 py-2 rounded-lg hover:bg-gray-200 z-10"
                            >
                                CANJEAR
                            </button>
                        </div>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center border-dashed">
                        <p className="text-zinc-500 text-sm">Completa tus 10 compras para desbloquear un cupón 2x1.</p>
                    </div>
                )}
            </div>
            ...
            <button 
              ...
              className="bg-zinc-800 text-white font-bold py-4 px-6 rounded-2xl flex items-center justify-center gap-2 w-full max-w-md hover:bg-zinc-700 transition-colors border border-zinc-700"
            >
              <QrCode size={20} />
              <span className="uppercase tracking-wide">Abrir mi Código QR</span>
            </button>
          </div>
        );
      ...
      case AppView.AI_CHEF:
         return (
            <div className="flex flex-col items-center w-full px-4 pb-24 animate-[slideUp_0.4s]">
                <AIChef />
            </div>
         );
  ...
  const BottomNav = () => {
    ...
    const navItems = [
        ...
        { view: AppView.MENU, icon: UtensilsCrossed, label: "Menú" },
        { view: AppView.AI_CHEF, icon: ChefHat, label: "Chef" },
        { view: AppView.CUSTOMER, icon: User, label: "Perfil" }, 
    ];
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-neutral-950/90 backdrop-blur-lg border-t border-zinc-900 px-6 py-4 flex justify-between items-center z-50 safe-area-bottom">
            {navItems.map((item) => (
                <button
                    key={item.label}
                    onClick={() => {
                        if (item.view === AppView.CUSTOMER && !profile) {
                            setCurrentView(AppView.REGISTER);
                        } else {
                    ...
                    className={`flex flex-col items-center gap-1 transition-colors relative ${
                        currentView === item.view ? 'text-orange-500' : 'text-zinc-500 hover:text-zinc-300'
                    }`}
                >
                    <div className="relative">
                        <item.icon size={24} strokeWidth={currentView === item.view ? 2.5 : 2} />
                        {/* Cart Badge for Menu Item */}
                        {item.view === AppView.MENU && cartCount > 0 && (
                            <span className="absolute -top-2 -right-2 bg-red-600 text-white text-[9px] w-5 h-5 flex items-center justify-center rounded-full border border-neutral-950 animate-pulse font-bold">
                                {cartCount}
                            </span>
                        )}
                    </div>
    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans selection:bg-orange-500 selection:text-white flex flex-col">
      ...
      <header className="p-4 flex justify-between items-center border-b border-zinc-900 bg-neutral-950/95 backdrop-blur-md sticky top-0 z-40 shadow-sm">
        <div 
            ...
            onClick={() => setCurrentView(AppView.HOME)}
        >
            <div className="w-9 h-9 bg-gradient-to-br from-orange-500 to-orange-700 rounded-lg flex items-center justify-center font-bold text-white brand-font text-2xl shadow-lg group-hover:scale-105 transition-transform">R</div>
            <span className="font-bold brand-font text-xl tracking-wide text-zinc-100">RAY BURGER</span>
        </div>
        
        {profile && currentView === AppView.CUSTOMER && (
             <button 
      ...
      {/* Main Content */}
      <main className="pt-6 flex flex-col items-center w-full flex-1">
        {renderContent()}
      </main>
      ...
      {currentView === AppView.ADMIN_SCANNER && (
        <Scanner 
            onScanSuccess={(id) => {
      ...
      {showQRModal && profile && (
        <div className="fixed inset-0 bg-black/90 z-[60] flex flex-col items-center justify-center p-4 backdrop-blur-md animate-[fadeIn_0.2s]">
            <div className="bg-white p-8 rounded-3xl max-w-sm w-full flex flex-col items-center text-center relative shadow-2xl">
                ...
                <h3 className="text-3xl font-bold text-black brand-font mb-1">CÓDIGO RAY</h3>
                <p className="text-zinc-500 text-sm mb-6 font-medium">Muestra este código al cajero</p>
                
                {/* Generated QR Code via API for demo purposes */}
                <div className="p-4 border-4 border-black rounded-2xl mb-6 shadow-inner bg-white">
                    <img 
                ...
                <button 
                    onClick={() => setShowQRModal(false)}
                    className="w-full bg-black text-white font-bold py-4 rounded-xl hover:bg-zinc-800 transition-colors shadow-lg"
                >
                    Listo, Cerrar
                </button>
            </div>
